...
export async function GET(request: Request, { params }: { params: { type: string } }) {
    ...
    if (match && match.groups) {
        ...
        if (selectedPagesMatch) {
            try {
                ...
                console.error(
                    `Could not parse selected pages input string to JSON Array. Original string -> ${selectedPagesMatch}`,
                );
            }
        }
    }
    ...
    const templates = await RestClient.getTemplates({
        type: params.type,
    ...
    if (currentlyUsed && currentlyUsed.Templates.length === 1) {
        ...
        let sameTemplateKey = Object.keys(templateRegistry).find(
            (x) => `${RENDERER_NAME}.${x}` === currentlyUsed!.Templates[0].Name,
        );
        if (sameTemplateKey) {
            tempaltesToSkip.push(sameTemplateKey);

            // remove the currently used template from the server category
            reactCategory.Templates = reactCategory.Templates.filter(
                (x) => x.Name !== currentlyUsed!.Templates[0].Name,
            );
        }
