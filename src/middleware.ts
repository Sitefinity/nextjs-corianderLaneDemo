...
export const templateRegex =
    /\/sf\/system\/(?<type>.*?)\/Default\.GetPageTemplates\(selectedPages=(?<selectedPages>.*?)\)/;
...
export async function middleware(request: NextRequest) {
    ...
    if (
        ...
        if (
            request.method === 'GET' &&
            (request.nextUrl.pathname.indexOf('/sf/system') !== -1 ||
                request.nextUrl.pathname.indexOf('/api/default') !== -1)
        ) {
...
function isAppStatusRequest(request: NextRequest) {
    return (
        request.nextUrl.pathname.toLowerCase() === '/appstatus' &&
